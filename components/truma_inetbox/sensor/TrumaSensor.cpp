...
void TrumaSensor::setup() {
  // Register callback for heater-related sensors
  if (this->type_ == TRUMA_SENSOR_TYPE::CURRENT_ROOM_TEMPERATURE ||
      this->type_ == TRUMA_SENSOR_TYPE::CURRENT_WATER_TEMPERATURE ||
      this->type_ == TRUMA_SENSOR_TYPE::TARGET_ROOM_TEMPERATURE ||
      this->type_ == TRUMA_SENSOR_TYPE::TARGET_WATER_TEMPERATURE ||
      this->type_ == TRUMA_SENSOR_TYPE::HEATING_MODE ||
      this->type_ == TRUMA_SENSOR_TYPE::ELECTRIC_POWER_LEVEL ||
      this->type_ == TRUMA_SENSOR_TYPE::ENERGY_MIX ||
      this->type_ == TRUMA_SENSOR_TYPE::OPERATING_STATUS ||
      this->type_ == TRUMA_SENSOR_TYPE::HEATER_ERROR_CODE) {
  ...
  // Register callback for aircon-related sensors
  if (this->type_ == TRUMA_SENSOR_TYPE::AIRCON_TARGET_TEMPERATURE ||
      this->type_ == TRUMA_SENSOR_TYPE::AIRCON_CURRENT_TEMPERATURE ||
      this->type_ == TRUMA_SENSOR_TYPE::AIRCON_MODE ||
      this->type_ == TRUMA_SENSOR_TYPE::AIRCON_VENT_MODE) {
    this->parent_->get_aircon_manual()->add_on_message_callback([this](const StatusFrameAirconManual *status_aircon) {
      switch (this->type_) {
        case TRUMA_SENSOR_TYPE::AIRCON_TARGET_TEMPERATURE:
          this->publish_state(temp_code_to_decimal(status_aircon->target_temp_aircon));
          break;
        case TRUMA_SENSOR_TYPE::AIRCON_CURRENT_TEMPERATURE:
          this->publish_state(temp_code_to_decimal(status_aircon->current_temp_aircon));
          break;
        case TRUMA_SENSOR_TYPE::AIRCON_MODE:
          this->publish_state(static_cast<float>(status_aircon->mode));
          break;
        case TRUMA_SENSOR_TYPE::AIRCON_VENT_MODE:
          this->publish_state(static_cast<float>(status_aircon->vent_mode));
          break;
        default:
          break;
      }
